...
| --- | --- | --- |
| Backend pytest suite | 33 tests pass. | FR-01 through FR-13, NFR-02 |
...
| Edge-case tests | Duplicate email, invalid login, invalid quantity, empty cart, duplicate order key, invalid token, unauthorized kitchen access, and invalid status are covered. | FR-03, FR-05, FR-07, FR-08, FR-11, FR-12, FR-13 |

## Testing Pyramid Summary

| Layer | Count | Evidence |
| --- | ---: | --- |
| Unit | 16 | Service-level business rule and boundary tests. |
| Integration | 17 | FastAPI `TestClient` API contract tests. |
